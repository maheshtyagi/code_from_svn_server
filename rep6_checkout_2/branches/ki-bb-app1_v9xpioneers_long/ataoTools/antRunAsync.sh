 #!/bin/sh

ERROR_ENV_SET=1

envProfile=bootstrap.sh

# Try pulling in updated .profile settings so startup will work correctly
 if [ -f "${envProfile}" ] ; then
    . "${envProfile}"
 else
    echo "$0: ERROR: ${envProfile} not found!"
    exit 1
 fi

 # If ANTRUN_NOHUP environment variable is set to true/yes/nohup, run the
 # executable prefixed with "nohup" making it immune to logging out.
 case $ANTRUN_NOHUP in
    true|yes|nohup) ANTRUN_NOHUP=nohup ;;
    *) unset ANTRUN_NOHUP ;;
 esac

 # JBOSS_HOME environment variable must be set


 if [ -n "$JBOSS_HOME" ]
 then
    cd "$JBOSS_HOME"
    $ANTRUN_NOHUP $@ 1>$JBOSS_HOME/jboss.out 2>$JBOSS_HOME/jboss.error &
    exit 0
 else
    echo "$0: ERROR: variable JBOSS_HOME must be set!"
    exit $ERROR_ENV_SET
 fi


if [ -z "$BAMBOO_HOME" ]; then
    echo "Please set BAMBOO_HOME"
    exit $ERROR_ENV_SET
fi

if [ -z "$ANT_OPTS" ]; then
    echo "Please set ANT_OPTS"
    exit $ERROR_ENV_SET
fi

if [ -z "$JAVA_HOME" ]; then
    echo "Please set JAVA_HOME"
    exit $ERROR_ENV_SET
fi

if [ -z "$ANT_HOME" ]; then
    echo "Please set ANT_HOME"
    exit $ERROR_ENV_SET
fi

if [ -z "$HOSTTYPE" ]; then
    echo "Please set HOSTTYPE"
    exit $ERROR_ENV_SET
fi


if [ -z "$PATH" ]; then
    echo "Please set PATH"
    exit $ERROR_ENV_SET
fi

if [ -z "$LD_LIBRARY_PATH" ]; then
    echo "Please set LD_LIBRARY_PATH"
    exit $ERROR_ENV_SET
fi
