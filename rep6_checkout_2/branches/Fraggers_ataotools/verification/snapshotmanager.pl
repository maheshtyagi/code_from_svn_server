...
sub create_snapshot {
   ...
   foreach (@$vm_views) {
      ...
      #Find if the snapshot exsists
      if(defined $_->snapshot) {
               ($ref, $nRefs) = find_snapshot_name ($_->snapshot->rootSnapshotList,
                                                    $snapshot_name);
      }
     
      #if the snapshot count does exists then 
      #die and exit out of the operation.
      if ( $nRefs >= 1 ) {
          Util::trace(0,"\n Snapshot exists with name"
                                 ." $snapshot_name in Virtual Machine ". $_->name
                                 ." under host ". $hostname
                                 ."\n");
          die"Cannot complete the operation";
      } 
      
      #Do the creation process if the snapshot does not exists.
