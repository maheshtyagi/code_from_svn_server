...
sub poweron_vm {
   foreach (@$vm_views) {
      ...
      eval {
         ...
         #Added a check to get the state of the VM before powering on.
         #print ("The power state is". $_->runtime->powerState->val."\n");
         if ( $_->runtime->powerState->val eq 'poweredOn') {

              Util::trace(0, "\nvirtual machine '" . $_->name .
                         "' under host $hostname  already powered on..not powering on \n");
      ...
      if ($@) { 
         if (ref($@) eq 'SoapFault') {
            ...
            if (ref($@->detail) eq 'NotSupported') {
               ...
               die "Cannot complete the operation";
            }
...
sub poweroff_vm {
   foreach (@$vm_views) {
      ...
      eval {
           ...
           #Code Added to check if the VM are already
           #in the powered down state.Do not power down the 
           #VM if the VMs are already powered down.  
          ...
          if ( $_->runtime->powerState->val eq 'poweredOn') {
              ...
              Util::trace (0, "\nvirtual machine '" . $_->name .
                       "' under host $hostname is already powered off..Not powering off and continuing ");
          }
      };
